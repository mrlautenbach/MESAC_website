    ...
    <li className="card p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted">
            ...
            {disabled && <span className="ml-2 badge bg-danger-tint text-danger">Disabled</span>}
          </p>
        </div>
      ...
      {showReset && (
        <form action={resetAction} className="mt-3 flex flex-wrap items-end gap-2 border-t border-border pt-3">
          ...
          <div>
            <label htmlFor={`${fid}-newPassword`} className="field-label">New password (leave blank to auto-generate)</label>
            <input id={`${fid}-newPassword`} name="newPassword" type="text" minLength={10} className="field-input w-64" />
          </div>
      ...
      {resetState?.ok && (
        <div className="mt-2 bg-success-tint px-3 py-2 text-sm text-success">
          Password reset.
          {resetState.tempPassword && (
            <>
              {" "}
              New temporary password:{" "}
              <code className="bg-card px-2 py-1 font-mono text-foreground">{resetState.tempPassword}</code>
            </>
          )}{" "}
